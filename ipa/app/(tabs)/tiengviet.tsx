...
// Data Phụ Âm, Nguyên Âm giữ nguyên không thay đổi
const VOWEL_TONES: Record<string, string[]> = {
  'a': ['a', 'á', 'à', 'ả', 'ã', 'ạ'], 'o': ['o', 'ó', 'ò', 'ỏ', 'õ', 'ọ'], 'ô': ['ô', 'ố', 'ồ', 'ổ', 'ỗ', 'ộ'],
  'ơ': ['ơ', 'ớ', 'ờ', 'ở', 'ỡ', 'ợ'], 'e': ['e', 'é', 'è', 'ẻ', 'ẽ', 'ẹ'], 'ê': ['ê', 'ế', 'ề', 'ể', 'ễ', 'ệ'],
  'i': ['i', 'í', 'ì', 'ỉ', 'ĩ', 'ị'], 'y': ['y', 'ý', 'ỳ', 'ỷ', 'ỹ', 'ỵ'], 'u': ['u', 'ú', 'ù', 'ủ', 'ũ', 'ụ'], 'ư': ['ư', 'ứ', 'ừ', 'ử', 'ữ', 'ự'],
};
const CONSONANTS = [
  { letter: 'b', vowels: ['a', 'o', 'ô', 'ơ', 'e', 'ê', 'i', 'y', 'u', 'ư'] }, { letter: 'c', vowels: ['a', 'o', 'ô', 'ơ', 'u', 'ư'] }, 
  { letter: 'ch', vowels: ['a', 'o', 'ô', 'ơ', 'e', 'ê', 'i', 'u', 'ư'] }, { letter: 'd', vowels: ['a', 'o', 'ô', 'ơ', 'e', 'ê', 'i', 'y', 'u', 'ư'] },
  { letter: 'đ', vowels: ['a', 'o', 'ô', 'ơ', 'e', 'ê', 'i', 'u', 'ư'] }, { letter: 'g', vowels: ['a', 'o', 'ô', 'ơ', 'u', 'ư'] }, 
  { letter: 'gh', vowels: ['e', 'ê', 'i'] }, { letter: 'gi', vowels: ['a', 'o', 'ô', 'ơ', 'e', 'u', 'ư'] }, 
  { letter: 'h', vowels: ['a', 'o', 'ô', 'ơ', 'e', 'ê', 'i', 'y', 'u', 'ư'] }, { letter: 'k', vowels: ['e', 'ê', 'i', 'y'] }, 
  { letter: 'kh', vowels: ['a', 'o', 'ô', 'ơ', 'e', 'ê', 'i', 'u', 'ư'] }, { letter: 'l', vowels: ['a', 'o', 'ô', 'ơ', 'e', 'ê', 'i', 'y', 'u', 'ư'] },
  { letter: 'm', vowels: ['a', 'o', 'ô', 'ơ', 'e', 'ê', 'i', 'y', 'u', 'ư'] }, { letter: 'n', vowels: ['a', 'o', 'ô', 'ơ', 'e', 'ê', 'i', 'y', 'u', 'ư'] },
  { letter: 'ng', vowels: ['a', 'o', 'ô', 'ơ', 'u', 'ư'] }, { letter: 'ngh', vowels: ['e', 'ê', 'i'] },
  { letter: 'nh', vowels: ['a', 'o', 'ô', 'ơ', 'e', 'ê', 'i', 'y', 'u', 'ư'] }, { letter: 'p', vowels: ['a', 'e', 'ê', 'i', 'o', 'ô', 'ơ', 'u', 'ư'] }, 
  { letter: 'ph', vowels: ['a', 'o', 'ô', 'ơ', 'e', 'ê', 'i', 'u', 'ư'] }, { letter: 'qu', vowels: ['a', 'e', 'ê', 'i', 'y', 'ơ'] }, 
  { letter: 'r', vowels: ['a', 'o', 'ô', 'ơ', 'e', 'ê', 'i', 'u', 'ư'] }, { letter: 's', vowels: ['a', 'o', 'ô', 'ơ', 'e', 'ê', 'i', 'u', 'ư'] },
  { letter: 't', vowels: ['a', 'o', 'ô', 'ơ', 'e', 'ê', 'i', 'y', 'u', 'ư'] }, { letter: 'th', vowels: ['a', 'o', 'ô', 'ơ', 'e', 'ê', 'i', 'u', 'ư'] },
  { letter: 'tr', vowels: ['a', 'o', 'ô', 'ơ', 'e', 'ê', 'i', 'u', 'ư'] }, { letter: 'v', vowels: ['a', 'o', 'ô', 'ơ', 'e', 'ê', 'i', 'y', 'u', 'ư'] },
        ...
        <View style={[styles.gv_leftColumn, { borderRightColor: colors.border }]}>
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.gv_leftScroll}>
            {CONSONANTS.map((item) => {
              ...
              return (
                <TouchableOpacity key={item.letter} style={[ styles.gv_consonantBtn, { backgroundColor: isActive ? '#38BDF8' : colors.card, borderColor: isActive ? '#0284C7' : colors.border } ]} onPress={() => handleSelectLetter(item.letter)}>
...
const TapDocGame = ({ onBack }: { onBack: () => void }) => {
  ...
  useFocusEffect(useCallback(() => { AsyncStorage.getItem('childName').then(n => { if(n) setChildName(n); }); loadAllData(); }, []));

  const loadAllData = async () => {
    try {
      const storedHidden = await AsyncStorage.getItem('@media_hidden');
      const hiddenIds = storedHidden ? JSON.parse(storedHidden) : [];
      const storedNames = await AsyncStorage.getItem('@media_names');
      const customNames = storedNames ? JSON.parse(storedNames) : {};

      const baseTapDoc = TAP_DOC_DATA
         .map((item, i) => ({ id: `def_tap_doc_${i}`, image: item.image, word: customNames[`def_tap_doc_${i}`] || item.word }))
         .filter(item => !hiddenIds.includes(item.id));

      const storedData = await AsyncStorage.getItem('@kho_du_lieu_cua_be');
      const customTapDoc = storedData ? (JSON.parse(storedData).tap_doc || [])
         .filter((i:any) => !hiddenIds.includes(i.id))
         .map((i:any) => ({ id: i.id, image: {uri: i.uri}, word: customNames[i.id] || i.name })) : [];

      const combinedData = [...baseTapDoc, ...customTapDoc];
      setAllData(combinedData);
      if (combinedData.length > 0) { unseenItems.current = [...combinedData]; pickNext(combinedData, [...combinedData]); }
    } catch (e) {}
  };
  ...
  const pickNext = (fullList: any[], currentUnseen: any[]) => {
    ...
    setCurrentItem(currentUnseen[randomIndex]);
  ...
  const handleSpeak = async () => {
    if (currentItem && currentItem.word) {
      try {
        const { sound } = await Audio.Sound.createAsync({ uri: `https://translate.google.com/translate_tts?ie=UTF-8&q=${encodeURIComponent(currentItem.word)}&tl=vi&client=tw-ob` });
      ...
      <View style={styles.td_mainContent}>
        {allData.length === 0 ? (
          <View style={styles.td_emptyContainer}>
            ...
            <Text style={styles.td_emptyText}>Kho ảnh đang trống trơn!</Text><Text style={styles.td_emptySubText}>Đại ca vào cài đặt bật ảnh lên nhé!</Text>
          </View>
        ) : currentItem ? (
          <>
            ...
            <View style={[styles.td_wordContainer, { flexDirection: 'row', alignItems: 'center' }]}>
              ...
              <TouchableOpacity style={{ backgroundColor: '#38BDF8', padding: 15, borderRadius: 50, elevation: 5, borderWidth: 3, borderColor: '#0284C7' }} onPress={handleSpeak}>
                <Ionicons name="volume-high" size={45} color="white" />
              </TouchableOpacity>
            </View>
            <TouchableOpacity style={styles.td_nextBtn} onPress={() => pickNext(allData, unseenItems.current)}><Text style={styles.td_nextBtnText}>Đổi Hình Khác 🎲</Text></TouchableOpacity>
          </>
        ) : null}
      </View>
    </View>
...
const BaiTapGame = ({ onBack }: { onBack: () => void }) => {
  const { colors } = useTheme();
  const [leftItems, setLeftItems] = useState<any[]>([]); const [rightItems, setRightItems] = useState<any[]>([]);
  ...
  const unseenGioi = useRef([...GIOI_VIDEOS]); const unseenTot = useRef([...TOT_VIDEOS]); const unseenCanCoGan = useRef([...CAN_CO_GAN_VIDEOS]);
  ...
  const startNewGame = async () => {
    try {
      ...
      const baseTapDoc = TAP_DOC_DATA.map((item, i) => ({ id: `def_tap_doc_${i}`, image: item.image, word: customNames[`def_tap_doc_${i}`] || item.word })).filter(item => !hiddenIds.includes(item.id));
      const storedData = await AsyncStorage.getItem('@kho_du_lieu_cua_be');
      const customTapDoc = storedData ? (JSON.parse(storedData).tap_doc || []).filter((i:any) => !hiddenIds.includes(i.id)).map((i:any) => ({ id: i.id, image: {uri: i.uri}, word: customNames[i.id] || i.name })) : [];

      const combinedData = [...baseTapDoc, ...customTapDoc];
      if (combinedData.length < 5) { Alert.alert('Thiếu dữ liệu', 'Đại ca bật thêm hình Tập Đọc trong Cài Đặt (ít nhất 5 tấm) để chơi nối chữ nha!'); return; }
  ...
  const panResponder = useRef(
    PanResponder.create({
      onStartShouldSetPanResponder: () => true, onStartShouldSetPanResponderCapture: () => true, onMoveShouldSetPanResponder: () => true, onMoveShouldSetPanResponderCapture: () => true,
        ...
        if (currentPathRef.current) {
          ...
          if (hitKey) {
            ...
            if (newCompleted.length === 5) {
              let dung = 0; newCompleted.forEach(p => { const s = dotLayouts.current[p.startKey]; const e = dotLayouts.current[p.endKey]; if (s && e && s.word === e.word) dung += 2; });
              setScore(dung);
              let origVid = dung >= 9 ? GIOI_VIDEOS : dung >= 6 ? TOT_VIDEOS : CAN_CO_GAN_VIDEOS;
              let unseenRef = dung >= 9 ? unseenGioi : dung >= 6 ? unseenTot : unseenCanCoGan;
              if (origVid.length > 0) {
                  if (unseenRef.current.length === 0) unseenRef.current = [...origVid];
                  const randIdx = Math.floor(Math.random() * unseenRef.current.length);
                  setRandomVideo(unseenRef.current[randIdx]); unseenRef.current.splice(randIdx, 1);
              }
              setTimeout(() => { if (origVid.length > 0) setShowVideoPopup(true); }, 1000); 
            }
          }
        }
      }
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.bg }]}>
      ...
      <View ref={gameAreaRef} onLayout={() => setTimeout(measureAllDots, 100)} style={styles.bt_gameArea}>
        <View style={StyleSheet.absoluteFill} pointerEvents="none">
          <Svg height="100%" width="100%">
            {completedPathsState.map((p, index) => <Path key={index} d={p.points.map((pt, i) => `${i === 0 ? 'M' : 'L'} ${pt.x} ${pt.y}`).join(' ')} stroke={p.color} strokeWidth="6" fill="none" strokeLinecap="round" strokeLinejoin="round" />)}
            {currentPathState && <Path d={currentPathState.points.map((pt, i) => `${i === 0 ? 'M' : 'L'} ${pt.x} ${pt.y}`).join(' ')} stroke={currentPathState.color} strokeWidth="6" fill="none" strokeLinecap="round" strokeLinejoin="round" />}
          </Svg>
        </View>
        <View style={styles.bt_column} pointerEvents="none">
          {leftItems.map((item, index) => {
            const dotKey = `left_${index}`; if (!dotLayouts.current[dotKey]) dotLayouts.current[dotKey] = { x: 0, y: 0, word: item.word, type: 'left' };
            return ( <View key={dotKey} style={styles.bt_itemRow}><Image source={item.image} style={styles.bt_image} resizeMode="cover" /><View ref={(r) => { dotRefs.current[dotKey] = r; }} style={styles.bt_dot} onLayout={measureAllDots} /></View> );
          })}
        </View>
        <View style={styles.bt_column} pointerEvents="none">
          {rightItems.map((item, index) => {
            const dotKey = `right_${index}`; if (!dotLayouts.current[dotKey]) dotLayouts.current[dotKey] = { x: 0, y: 0, word: item.word, type: 'right' };
      ...
      {score !== null && (
        <View style={styles.bt_resultModal}><Text style={styles.bt_resultText}>Điểm của bé: {score}/10 {score === 10 ? '🎉' : '👏'}</Text><TouchableOpacity style={styles.bt_replayBtn} onPress={startNewGame}><Text style={styles.bt_replayBtnText}>Chơi lại 🔄</Text></TouchableOpacity></View>
      )}
      <Modal visible={showVideoPopup && randomVideo !== null} transparent={true} animationType="fade">
        <View style={styles.bt_videoOverlay}><View style={styles.bt_videoWrapper}><Video source={randomVideo} style={styles.bt_smallVideo} resizeMode={ResizeMode.CONTAIN} shouldPlay onPlaybackStatusUpdate={(s) => { if (s.isLoaded && s.didJustFinish) setShowVideoPopup(false); }} /><TouchableOpacity style={styles.bt_skipVideoBtn} onPress={() => setShowVideoPopup(false)}><Text style={styles.bt_skipVideoText}>X</Text></TouchableOpacity></View></View>
      </Modal>
    </View>
  );
};
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.bg }]}>
      ...
      <View style={styles.menuContainer}>
        ...
        <TouchableOpacity style={[styles.menuCard, { borderColor: '#FACC15', backgroundColor: '#FEFCE8' }]} onPress={() => setCurrentView('bai_tap')}><Text style={styles.menuIcon}>✏️</Text><View><Text style={[styles.menuTitle, { color: '#854D0E' }]}>Nối Từ Cùng Bé</Text><Text style={{ color: '#EAB308', fontSize: 16 }}>Nối hình ảnh với chữ tương ứng</Text></View></TouchableOpacity>
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  gv_consonantBtn: { height: 80, justifyContent: 'center', alignItems: 'center', borderWidth: 2, borderRadius: 15, marginBottom: 10 },
  ...
  gv_vowelGroup: { backgroundColor: 'white', borderRadius: 20, padding: 10, marginBottom: 20, borderWidth: 2, borderColor: '#E2E8F0' },
  ...
  td_imageCard: { width: width * 0.4, height: width * 0.4, borderRadius: 25, borderWidth: 4, overflow: 'hidden', marginBottom: 30, alignItems: 'center', justifyContent: 'center' },
  ...
  td_nextBtn: { flexDirection: 'row', backgroundColor: '#3B82F6', paddingVertical: 18, paddingHorizontal: 40, borderRadius: 20, alignItems: 'center' },
  ...
  bt_dot: { width: 25, height: 25, borderRadius: 15, backgroundColor: '#9CA3AF', borderWidth: 3, borderColor: '#4B5563' },
  bt_resultModal: { position: 'absolute', top: '40%', left: '10%', right: '10%', backgroundColor: 'white', padding: 30, borderRadius: 20, alignItems: 'center', zIndex: 999 },
  ...
  bt_videoWrapper: { width: width * 0.5, height: width * 0.5 * (9/16), backgroundColor: '#000', borderRadius: 20, overflow: 'hidden' },
