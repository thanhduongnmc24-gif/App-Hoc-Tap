...
export default function LearningScreen() {
  ...
  useEffect(() => {
    const preloadAssets = async () => {
      try { await Asset.loadAsync(ALL_IMAGES); } catch (e) {}
    };
    preloadAssets();
  }, []);
  ...
  const loadCustomMedia = async () => {
    try {
      ...
      console.log('Lỗi lấy ảnh tính điểm:', e);
    }
  };
  ...
  const createNewProblems = (limit: number, type: string) => {
    ...
    let hasZeroResultProblem = false; 
    attempts = 0;
    ...
    while (subtractions.length < numSub && attempts < 100) {
      ...
      if (!usedCombos.has(comboKey)) {
        ...
        if (num1 === num2) hasZeroResultProblem = true;
      }
    }

    while (subtractions.length < numSub) {
      let num1 = Math.floor(Math.random() * safeLimit) + 1;
      if (hasZeroResultProblem && num1 === 1) num1 = 2; 
      let num2 = hasZeroResultProblem ? Math.floor(Math.random() * (num1 - 1)) + 1 : Math.floor(Math.random() * num1) + 1;
      subtractions.push({ id: 0, num1, num2, operator: '-', userAnswer: '' });
      if (num1 === num2) hasZeroResultProblem = true;
    }
  ...
  const handleNumpadOK = () => setActiveInputIndex(null);
  ...
  const handleSubmit = () => {
    ...
    if (!isCompleted) {
      ...
      else Alert.alert('Khoan đã!', `${childName} chưa làm xong hết 10 câu kìa!`, [{ text: 'Dạ vâng', style: 'default' }]); 
      return;
    }
    ...
    problems.forEach(p => {
      ...
      if (expectedAnswer === parseInt(p.userAnswer, 10)) currentScore += 1;
    });

    setScore(currentScore);
    setIsSubmitted(true);
    
    let selectedImageArray: any[] = [];
  ...
  const Numpad = () => (
    <View style={styles.numpadContainer}>
      ...
      <View style={styles.numpadRow}>
        ...
        <TouchableOpacity style={[styles.numpadBtn, { backgroundColor: '#10B981' }]} onPress={handleNumpadOK}><Text style={[styles.numpadText, { color: 'white' }]}>OK</Text></TouchableOpacity>
      </View>
      <View style={styles.submitContainer}>
        <TouchableOpacity style={styles.submitBtnRight} onPress={handleSubmit} activeOpacity={0.8}><Text style={styles.submitBtnRightText}>🏆 CHẤM ĐIỂM 🏆</Text></TouchableOpacity>
      </View>
    </View>
  );
  ...
  const ScoreBoard = () => (
    <View style={styles.scoreBoardContainer}>
      {score === 10 && <ConfettiCannon count={200} origin={{x: -10, y: 0}} fallSpeed={2500} fadeOut={true} autoStart={true} />}
      ...
      <Modal visible={showVideoPopup && randomVideo !== null} transparent={true} animationType="fade">
        <View style={styles.videoOverlay}>
          <View style={styles.videoWrapper}>
            <Video source={randomVideo} style={styles.smallVideo} resizeMode={ResizeMode.CONTAIN} shouldPlay
